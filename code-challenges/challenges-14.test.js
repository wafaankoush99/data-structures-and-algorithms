...
const toTitleCase = (arr) => {
  ...
  let arr2 = [];
  arr.forEach(it => {
    let u = it.charAt(0).toUpperCase();
    let o = it.replace(/^[A-Z]{1}/gi, u);
    arr2.push(o);
  });
  return arr2;
};
...
const detectTicTacToeWin = (board) => {
  ...
  winners.push(board[0]);
  winners.push(board[1]);
  winners.push(board[2]);
  let it0 = [];
  let it1 = [];
  let it2 = [];
  board.forEach(it=>{
