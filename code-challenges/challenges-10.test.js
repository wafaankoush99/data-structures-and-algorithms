...
const count = (target, input) => {
  ...
  let b = s.join('');
  let u = b.split('');
  let k = u.map(i=>Number(i));
  let v = k.filter(function(it){
    if (it === target){
      return it;
    }
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here..
  return input.map(function(it1) {
    let a = it1.filter(it2 => typeof it2 === 'number' && it2 % 5 === 0);
    // console.log(a)
    let c = a.map(n => Math.pow(2, n));
    return c;
  });
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  // console.log(data);
  let y = data.filter(function(it){
    // console.log(it.name)
    if (it.gender==='male' || it.gender==='female'){
      return it;
    }
  });
  let s = y.map(function(it2){
    return it2.name;
  });
  // console.log(s)
  let p = s.join(' and ');
...
let findShortest = (data) => {
  ...
  let s = data.map(function(it2){
    return it2.height;
  });
  let k = s.map(i=>Number(i));
  // console.log(k)
  let u = Math.min.apply( Math, k );
  // console.log(u)

  let m = '';
  data.forEach(function(it){
    // console.log(it.height)
    if (it.height === u.toString()){
      // console.log(it.name);
      m = it.name;
      // console.log(m)
    }
