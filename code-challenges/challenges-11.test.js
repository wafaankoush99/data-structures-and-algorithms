...
const validatePin = (pin) => {
  // Solution code here...
  let reg = /[0-9]/gi;
  if (pin.toString().match(reg) && pin.toString().length === 4) {
    return true;
  }
  else return false;
...
const validateEmail = (email) => {
  // Solution code here...
  let reg = /@/gi;
  let s1 = email.match(reg);
  if (s1) {
    let reg2 = /\.(?=[A-Za-z])/g;
    let s2 = email.match(reg2);
    if (s2 === null) { return false; }
    if (s2.length <= 2) {
      let reg3 = /[a-zA-Z](\@)/gi;
      let s3 = email.match(reg3);
      if (s3) {
        let reg4 = /['.net','.com','.org']$/gi;
        let s4 = email.match(reg4);
        if (s4) {
          return true;
        }
        if (!s4) return false;
    ...
    if (s2.length > 2 || s2 === null) { return false; }
  }
  if (!s1) return false;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return /^(\([0-9]{3}\)|\d{3})[\s-]?[0-9]{3}[\s-]?[0-9]{4}$/im.test(phoneNumber);
};
