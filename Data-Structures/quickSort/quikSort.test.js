...
describe('Happy Path', () => {
    describe('Shoule retirn sorted array in these cases', () => {
        it('Normal case', () => {
            ...
            expect(sort(normalCase, 0 , normalCase.length-1)).toEqual([4, 8, 15, 16, 23, 42])
        })
        it('Reverse-sorted', () => {
            let ReverseSorted = [20, 18, 12, 8, 5, -2];
            expect(sort(ReverseSorted, 0 , ReverseSorted.length-1)).toEqual([-2, 5, 8, 12, 18, 20])
        })
        it('Few uniques', () => {
            let FewUniques = [5, 12, 7, 5, 5, 7];
            expect(sort(FewUniques, 0 , FewUniques.length-1)).toEqual([5, 5, 5, 7, 7, 12])
        })
        it('Nearly-sorted', () => {
            let NearlySorted = [2, 3, 5, 7, 13, 11];
            expect(sort(NearlySorted, 0 , NearlySorted.length-1)).toEqual([2, 3, 5, 7, 11, 13])
        })
    })
